...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-28-1.pdf}
...
The second option comes from the research vignette. The Tran et al. \citep{tran_you_2014} has two variables where the authors have conducted one-way ANOVAs. I will demonstrate one (\emph{Accurate}) in this lecture; the second is suggested as homework.
...
\begin{figure}
\centering
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-48-1.pdf}
\caption{\label{fig:unnamed-chunk-48}Graphical illustration of ``between groups'' variation}
\end{figure}

\begin{figure}
\centering
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-49-1.pdf}
\caption{\label{fig:unnamed-chunk-49}Graphical illustration of ``within groups'' variation}
\end{figure}
...
\NormalTok{sdControl}\SpecialCharTok{\^{}}\DecValTok{2}\CommentTok{\#the standard deviation of the control group, when squared should equal the variance reported in the next chunk}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{shapiro }\OtherTok{\textless{}{-}}\NormalTok{ accSIM30 }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{shapiro\_test}\NormalTok{(Accurate)}
\NormalTok{shapiro}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{levene }\OtherTok{\textless{}{-}} \FunctionTok{leveneTest}\NormalTok{ (Accurate }\SpecialCharTok{\textasciitilde{}}\NormalTok{ COND, accSIM30, }\AttributeTok{center=}\NormalTok{mean)}
\NormalTok{levene}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{summary}\NormalTok{ (omnibus) }\CommentTok{\#ANOVA output}
\end{Highlighting}
\end{Shaded}
...
The values we see map onto those we calculated by hand. Our \(SS_M\) (9.432) plus \(SS_R\) (30.246) sum to equal the \(SS_T\) (39.678) we calculated by hand.

Dividing the two sums of squares by their respective degrees of freedom produces the sums of means squared.

Then, dividing the \(MS_M\) (COND) by \(MS_R\) (4.716/0.348) provides the \emph{F} value. By using a table of \emph{F} critical values, we already knew that our \emph{F} value exceeded the value in the table of critical values. Here we see that \emph{p} = 0.000.
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-83-1.pdf} \includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-83-2.pdf} \includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-83-3.pdf} \includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-83-4.pdf}
...
\NormalTok{SSMomnibus }\SpecialCharTok{/}\NormalTok{ (SSMomnibus }\SpecialCharTok{+} \FloatTok{30.246}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#Calculated by using the numeric values}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#Optiosn should produce the same result}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{etaSquared}\NormalTok{(omnibus)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{eta2\_to\_f}\NormalTok{(.}\DecValTok{238}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{pwr.anova.test}\NormalTok{ (}\AttributeTok{k =} \DecValTok{3}\NormalTok{, }\AttributeTok{f =}\NormalTok{ .}\DecValTok{5589}\NormalTok{, }\AttributeTok{sig.level =}\NormalTok{ .}\DecValTok{05}\NormalTok{, }\AttributeTok{power =}\NormalTok{ .}\DecValTok{80}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
If we were unsure about what to expect in a sample size, we could take a guess. I like to be on the safe(r) side and go with a smaller effect. What would happen if we had a Cohen's \emph{f} that represented a small effect?

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{pwr.anova.test}\NormalTok{ (}\AttributeTok{k =} \DecValTok{3}\NormalTok{, }\AttributeTok{f =}\NormalTok{ .}\DecValTok{1}\NormalTok{, }\AttributeTok{sig.level =}\NormalTok{ .}\DecValTok{05}\NormalTok{, }\AttributeTok{power =}\NormalTok{ .}\DecValTok{80}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-99-1.pdf}
...
\NormalTok{df1omnibus}\OtherTok{\textless{}{-}} \FunctionTok{digits}\NormalTok{(}\FunctionTok{summary}\NormalTok{(omnibus)[[}\DecValTok{1}\NormalTok{]][}\DecValTok{1}\NormalTok{, }\StringTok{"Df"}\NormalTok{],}\DecValTok{0}\NormalTok{)}
\NormalTok{df1omnibus}
\NormalTok{df2omnibus}\OtherTok{\textless{}{-}} \FunctionTok{digits}\NormalTok{(}\FunctionTok{summary}\NormalTok{(omnibus)[[}\DecValTok{1}\NormalTok{]][}\DecValTok{2}\NormalTok{, }\StringTok{"Df"}\NormalTok{],}\DecValTok{0}\NormalTok{)}
\NormalTok{df2omnibus}
\NormalTok{SSMomnibus}\OtherTok{\textless{}{-}} \FunctionTok{digits}\NormalTok{(}\FunctionTok{summary}\NormalTok{(omnibus)[[}\DecValTok{1}\NormalTok{]][}\DecValTok{1}\NormalTok{, }\StringTok{"Sum Sq"}\NormalTok{],}\DecValTok{3}\NormalTok{)}
\NormalTok{SSMomnibus}
\NormalTok{SSRomnibus}\OtherTok{\textless{}{-}} \FunctionTok{digits}\NormalTok{(}\FunctionTok{summary}\NormalTok{(omnibus)[[}\DecValTok{1}\NormalTok{]][}\DecValTok{2}\NormalTok{, }\StringTok{"Sum Sq"}\NormalTok{],}\DecValTok{3}\NormalTok{)}
\NormalTok{SSRomnibus}
\NormalTok{SSTomnibus }\OtherTok{\textless{}{-}} \FunctionTok{digits}\NormalTok{((SSMomnibus }\SpecialCharTok{+}\NormalTok{ SSRomnibus),}\DecValTok{3}\NormalTok{)}
\NormalTok{SSTomnibus}
\NormalTok{MSMomnibus}\OtherTok{\textless{}{-}} \FunctionTok{digits}\NormalTok{(}\FunctionTok{summary}\NormalTok{(omnibus)[[}\DecValTok{1}\NormalTok{]][}\DecValTok{1}\NormalTok{, }\StringTok{"Mean Sq"}\NormalTok{],}\DecValTok{3}\NormalTok{)}
\NormalTok{MSMomnibus}
\NormalTok{MSRomnibus}\OtherTok{\textless{}{-}} \FunctionTok{digits}\NormalTok{(}\FunctionTok{summary}\NormalTok{(omnibus)[[}\DecValTok{1}\NormalTok{]][}\DecValTok{2}\NormalTok{, }\StringTok{"Mean Sq"}\NormalTok{],}\DecValTok{3}\NormalTok{)}
\NormalTok{MSRomnibus}
\NormalTok{Fomnibus}\OtherTok{\textless{}{-}} \FunctionTok{digits}\NormalTok{(}\FunctionTok{summary}\NormalTok{(omnibus)[[}\DecValTok{1}\NormalTok{]][}\DecValTok{1}\NormalTok{, }\StringTok{"F value"}\NormalTok{],}\DecValTok{3}\NormalTok{)}
\NormalTok{Fomnibus}
\NormalTok{pomnibus}\OtherTok{\textless{}{-}} \FunctionTok{digits}\NormalTok{(}\FunctionTok{summary}\NormalTok{(omnibus)[[}\DecValTok{1}\NormalTok{]][}\DecValTok{1}\NormalTok{, }\StringTok{"Pr(\textgreater{}F)"}\NormalTok{],}\DecValTok{3}\NormalTok{)}
\NormalTok{pomnibus}
\end{Highlighting}
\end{Shaded}
...
The second option comes from the research vignette. The Ramdhani et al. \citeyearpar{ramdhani_affective_2018} article has two dependent variables (negative and positive evaluation) which are suitable for two-way ANOVAs. I will demonstrate a simulation of one of their 2X3 ANOVAs (negative) in this lecturette. The second dependent variable (positive) is suggested for the moderate level of difficulty.
...
\item
  ...
  \end{itemize}
\item
  Ramdhani, N., Thontowi, H. B., \& Ancok, D. (2018). Affective Reactions Among Students Belonging to Ethnic Groups Engaged in Prior Conflict. \emph{Journal of Pacific Rim Psychology, 1}2, e2. \url{https://doi.org/10.1017/prp.2017.22}
\end{itemize}
...
\textbf{ANOVA is for experiments} (or arguably closely related designs). As we get into the assumptions you'll see that it has some interesting ones (e.g., there should be an equal/equivalent number of cases per cell). To the degree that we violate these assumptions, we should locate alternative statistical approaches where these assumptions are relaxed.

\textbf{Factorial}: the term used when there are two or more independent variables (the factors). The factors could be between-groups, within-groups, repeated measures, or a combination of between and within.
...
\item
  \textbf{Related factorial design}: several IVs (factors/predictors) have been measured, but the same participants have been used in all conditions (repeated measures).
\item
  \textbf{Mixed design}: several IVs (factors/predictors) have been measured; some with different participants whereas others use the same participants. A combo of independent (between) and related (within) designs.
\item
...
\hypertarget{id}{%
\centering
\includegraphics[width=10.41667in,height=4.16667in]{images/factorial/modfigs.jpg}
\caption{Graphic representations of a moderated relationship?}\label{id}
}
\end{figure}
...
\CommentTok{\#ANOVAresults\textless{}{-}aov(Negative\textasciitilde{}Rater*Photo, Ramdhani\_df) \#runs the ANOVA {-}{-} I used this in the simulation to check my work}
\CommentTok{\#summary(ANOVAresults) \#ANOVA output}
\CommentTok{\#model.tables(ANOVAresults,"means") \#extracts the means for the 3 groups}
\end{Highlighting}
\end{Shaded}
...
Let's first examine the descriptive statistic (e.g., means of Negative) by group. We can use the \emph{describeBy()} function from the \emph{psych} package.
...
\FunctionTok{ggboxplot}\NormalTok{(Ramdhani\_df, }\AttributeTok{x =} \StringTok{"Rater"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Negative"}\NormalTok{, }\AttributeTok{color =} \StringTok{"Photo"}\NormalTok{,}\AttributeTok{xlab =} \StringTok{"Ethnicity of Rater"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Negative Reaction"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggboxplot}\NormalTok{(Ramdhani\_df, }\AttributeTok{x =} \StringTok{"Photo"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Negative"}\NormalTok{, }\AttributeTok{color =} \StringTok{"Rater"}\NormalTok{, }\AttributeTok{xlab =} \StringTok{"Photo Stimulus"}\NormalTok{,}
             \AttributeTok{ylab =} \StringTok{"Negative Reaction"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-112-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggline}\NormalTok{(Ramdhani\_df, }\AttributeTok{x =} \StringTok{"Rater"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Negative"}\NormalTok{, }\AttributeTok{color =} \StringTok{"Photo"}\NormalTok{, }\AttributeTok{xlab =} \StringTok{"Ethnicity of Rater"}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggline}\NormalTok{(Ramdhani\_df, }\AttributeTok{x =} \StringTok{"Photo"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Negative"}\NormalTok{, }\AttributeTok{color =} \StringTok{"Rater"}\NormalTok{, }\AttributeTok{xlab =} \StringTok{"Photo Stimulus"}\NormalTok{,}
             \AttributeTok{ylab =} \StringTok{"Negative Reaction"}\NormalTok{, }\AttributeTok{add =} \FunctionTok{c}\NormalTok{(}\StringTok{"mean\_se"}\NormalTok{, }\StringTok{"dotplot"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\(SS_R\) is error associated with within group variability -- If people are randomly assigned to treatment group there should be no other covariate (confounding variable) so that all SSR variability is \emph{uninteresting} for the research and treated as noise.
...
\hypertarget{relationship-between-ss_t-ss_m-and-ss_r.-in-case-its-not-clear}{%
\subsection{\texorpdfstring{Relationship between \(SS_T\), \(SS_M\), and \(SS_R\). In case it's not clear:}{Relationship between SS\_T, SS\_M, and SS\_R. In case it's not clear:}}\label{relationship-between-ss_t-ss_m-and-ss_r.-in-case-its-not-clear}}
...
\hypertarget{but-we-need-to-understand-the-ss-for-each-of-the-factors-and-their-product.}{%
\subsection{But we need to understand the SS for each of the factors and their product.}\label{but-we-need-to-understand-the-ss-for-each-of-the-factors-and-their-product.}}

\hypertarget{rater-main-effect}{%
\subsubsection{Rater Main Effect}\label{rater-main-effect}}

\(SS_a:Rater\) is calculated the same way as \(SS_M\) for one-way ANOVA, simply collapsing across Photo and calculating the \emph{marginal means} for Negative as a function of ethnicity of the Rater:
...
\(SS_b:Photo\) is calculated the same way as \(SS_M\) for one-way ANOVA, simply collapsing across Rater and calculating the \emph{marginal means} for Negative as a function of ethnicity reflected in the Photo stimulus:
...
\item
  ...
  \item
    ANOVA was really designed for the RCT; while we see it applied elsewhere\ldots argHHHHHHHH
  \end{itemize}
\item
  ...
  \item
    There are better options for analyzing data that has dependencies (i.e., repeated measures ANOVA\ldots but also dyadic, multilevel modeling)
  \end{itemize}
\item
...
\caption{Image of a flowchart showing that we are on the ``Evaluating assumptions'' portion of the decision-tree}
\end{figure}
...
We can start easy with \textbf{skew} and \textbf{kurtosis}.
...
Using guidelines from Kline \citeyearpar{kline_principles_2016}
* All skew is below 3;
* All kurtosis is below 8 (8 to 20 is considered to be \emph{extreme})
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-138-1.pdf}
So far so good -- these look normal. Let's examine a QQ plot. We want the dots (the \emph{residuals}), or what is leftover after the model is applied to line up on the diagonal line (within reason). This would indicate a normal distribution.
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-139-1.pdf}
We can formally test the distribution of the residuals with a Shapiro test. We want the associated \emph{p} value to be less than 0.05.
...
Levene's test is also applied to the ANOVA model; in this case I just wrote it out. Levene's requires a \emph{fully saturated model.} This means that the prediction model requires an interaction effect (not just two, non-interacting predictors).

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(car)}
\FunctionTok{leveneTest}\NormalTok{(Negative }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Rater}\SpecialCharTok{*}\NormalTok{Photo, }\AttributeTok{data =}\NormalTok{ Ramdhani\_df)}
\end{Highlighting}
\end{Shaded}
...
The \emph{F}-tests associated with the two-way ANOVA are the \emph{omnibus} -- giving the result for the main and interaction effects.
...
\caption{Image our place in the Two-Way ANOVA Decision-Tree.}
\end{figure}
...
Let's gather the \emph{F strings}''* from the above table.
Rater main effect: significant (\emph{F}{[}2, 105{]} = 8.077, \emph{p} \textless{} .001). Photo stimulus main effect: significant (\emph{F}{[}1, 105{]} = 19.346, \emph{p} \textless{} .001). Interaction effect: significant (\emph{F}{[}2, 105{]} = 5.696, \emph{p} = .004).
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-143-1.pdf} \includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-143-2.pdf} \includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-143-3.pdf} \includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-143-4.pdf}

\hypertarget{effect-sizes}{%
\subsubsection{Effect sizes}\label{effect-sizes}}

\textbf{Eta squared} is one of the most commonly used measures of effect. It refers to the proportion of variability in the DV/outcome variable that can be explained in terms of the IV/predictor. Traditional interpretive values are similar to the Pearson's \emph{r}:
...
More interpretive info can be found here: \url{https://imaging.mrc-cbu.cam.ac.uk/statswiki/FAQ/effectSize}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(lsr)}
\FunctionTok{etaSquared}\NormalTok{(TwoWay\_neg)}
\end{Highlighting}
\end{Shaded}
...
We can update our \emph{F} string:
Rater main effect: significant (\emph{F}{[}2, 105{]} = 8.077, \emph{p} \textless{} .001, \(\eta ^{2}\) = 0.107). Photo stimulus main effect: significant (\emph{F}{[}1, 105{]} = 19.346, \emph{p} \textless{} .001, \(\eta ^{2}\) = 0.127). Interaction effect: significant (\emph{F}{[}2, 105{]} = 5.696, \emph{p} = .004, \(\eta ^{2}\) = 0.075).
...
\emph{\ldots The next paragraph will have one of the follow-up options. We will add it later in the lesson}
...
When an interaction effect is significant (irrespective of the significance of one or more main effects), examination of \textbf{simple main effects} is a common statistical/explanatory approached that is used. The Two-Way ANOVA Decision Tree shows where we are in this process.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{etaSquared}\NormalTok{(Dayaknese\_simple, }\AttributeTok{anova =} \ConstantTok{FALSE}\NormalTok{ ) }\CommentTok{\#effect size for simple main effect can add "type = 1,2,3,4" to correspond with the ANOVA that was run}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{etaSquared}\NormalTok{(Madurese\_simple, }\AttributeTok{anova =} \ConstantTok{FALSE}\NormalTok{ ) }\CommentTok{\#effect size for simple main effect can add "type = 1,2,3,4" to correspond with the ANOVA that was run}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{etaSquared}\NormalTok{(Javanese\_simple, }\AttributeTok{anova =} \ConstantTok{FALSE}\NormalTok{ ) }\CommentTok{\#effect size for simple main effect can add "type = 1,2,3,4" to correspond with the ANOVA that was run}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-148-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{etaSquared}\NormalTok{(Dykn\_simple, }\AttributeTok{anova =} \ConstantTok{FALSE}\NormalTok{ ) }\CommentTok{\#effect size for simple main effect can add "type = 1,2,3,4" to correspond with the ANOVA that was run}
\end{Highlighting}
\end{Shaded}
...
\emph{F} {[}2, 51{]}) = 13.32, \emph{p} \textless{} .001, \(\eta ^{2}\) = 0.343.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{etaSquared}\NormalTok{(Mdrs\_simple, }\AttributeTok{anova =} \ConstantTok{FALSE}\NormalTok{ ) }\CommentTok{\#effect size for simple main effect can add "type = 1,2,3,4" to correspond with the ANOVA that was run}
\end{Highlighting}
\end{Shaded}
...
\emph{F} {[}2, 54{]} = 0.679, \emph{p} = .512, \(\eta ^{2}\) = 0.024.
...
\textbf{APA Write-up of the simple main effect of photo stimulus within rater ethnicity.} *This would be added to the write-up of the omnibus two-way ANOVA test.
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-157-1.pdf}

\hypertarget{post-hoc-comparisons}{%
\subsubsection{Post hoc comparisons}\label{post-hoc-comparisons}}

Another option is compare all possible cells. These are termed \emph{post hoc comparisons.} They are an alternative to simple main effects; you would not report both. The figure shows our place on the Two-Way ANOVA Decision Tree.
...
In Excel, I would sort my results by their \emph{p} values (low to high) and consider my thresshold (\emph{p} \textless{} .003) to determine which effects were statistically significant. Using the strictest criteria of \emph{p} \textless{} .0033, we would have four statistically significant values.
...
\subsubsection{Polynomial Trends}\label{polynomial-trends}}
...
\hypertarget{in-the-event-of-a-non-significant-interaction-but-one-or-more-significant-main-effects}{%
\section{In the event of a non-significant interaction, but one or more significant main effects}\label{in-the-event-of-a-non-significant-interaction-but-one-or-more-significant-main-effects}}

We now focus on the possibility that there might be significant main effects, but a non-significant interaction effect. We \textbf{only} interpret main effects when there is a non-significant interaction effect. Why? Because in the presence of a significant interaction effect, the main effect will not tell a complete story. \emph{(And, if we didn't specify a correct model, we still might have an incomplete story. But that's another issue.)} Here's where we are on the Decision-Tree.
...
I will demo how to do each as follow-up to a \emph{pretend} scenario where the main effect (but not the interaction) had been significant. I will write up the portion that would be inserted in an APA style results section.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{etaSquared}\NormalTok{(RaterMain)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggboxplot}\NormalTok{(Ramdhani\_df, }\AttributeTok{x =} \StringTok{"Rater"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Negative"}\NormalTok{, }\AttributeTok{xlab =} \StringTok{"Ethnicity of Rater"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Negative Reaction"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-167-1.pdf}
...
We generally try for \emph{orthogonal} contrasts so that the partitioning of variance is independent (clean, not overlapping). Planned contrasts are a great way to do this. Here's where we are in the Decision-Tree.
...
If you aren't SUPER DUPER careful about your order-of-operations in R, it can confuse objects, so I have named these contrasts \emph{main\_c1} and \emph{main\_c2} to remind myself that they refer to the main effect of ethnicity of the rater.
...
\item
  ...
  \tightlist
  \item
    piece of cake analogy\ldots once you take out a piece of the cake, you really can't put it back in
  \end{itemize}
\end{itemize}
...
\hypertarget{polynomial-trends-1}{%
\subsection{Polynomial Trends}\label{polynomial-trends-1}}

Polynomial contrasts let us see if there is a linear (or curvilinear) pattern to the data. To detect a trend, the data must be coded in an ascending order\ldots and it needs to be a sensible comparison. Here's where this would fall in our Decision-Tree.
...
\textbf{Rater.!} tests to see if there is a significant quadratic (curvilinear, one hump) trend. There is not: \emph{t} = -0.567, \emph{p} = .572.
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-172-1.pdf}

\hypertarget{my-final-apa-style-results-section}{%
\section{\texorpdfstring{My \emph{final} APA Style Results Section}{My final APA Style Results Section}}\label{my-final-apa-style-results-section}}
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-173-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(apaTables)}
\FunctionTok{apa.2way.table}\NormalTok{(}\AttributeTok{iv1 =}\NormalTok{ Rater, }\AttributeTok{iv2 =}\NormalTok{ Photo, }\AttributeTok{dv =}\NormalTok{ Negative, }\AttributeTok{data =}\NormalTok{ Ramdhani\_df, }\AttributeTok{landscape=}\ConstantTok{TRUE}\NormalTok{, }\AttributeTok{table.number =} \DecValTok{1}\NormalTok{, }\AttributeTok{filename=}\StringTok{"Table\_1\_MeansSDs.doc"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{apa.aov.table}\NormalTok{(TwoWay\_neg, }\AttributeTok{filename =} \StringTok{"Table\_2\_effects.doc"}\NormalTok{, }\AttributeTok{table.number =} \DecValTok{2}\NormalTok{, }\AttributeTok{type =} \StringTok{"II"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Field \citeyearpar{field_discovering_2012} suggests that it is safest to stick with Type III sums of squares . We apply the type to the model we create in the initial run. For more information, check out this explanation on \href{https://www.r-bloggers.com/2011/03/anova-\%E2\%80\%93-type-iiiiii-ss-explained/}{r-bloggers}.
...
\FunctionTok{Anova}\NormalTok{(TwoWay\_neg)}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{Anova}\NormalTok{(TwoWay\_neg, }\AttributeTok{type =}\StringTok{"III"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{etaSquared}\NormalTok{(TwoWay\_neg, }\AttributeTok{type=}\DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\emph{Power} is often our euphemistic way of asking, ``How large should my sample size be?''

Power defined is the ability of the test to detect statistical significance when there is such. It's represented formulaically as 1 - \emph{P})(Type II error). Power is traditionally set at 80\% (or .8)
...
\tightlist
\item
  In this case, we will rerun the statistic, grab both, and convert them to the \emph{f} (not the \(f^2\))
  ...
  \end{itemize}
\item
  In unknown case, you can substitute what you expect using Cohen's guidelines of .10, .25, and .40 as small, medium, and large (for the \emph{f}, not \(F^2\))
\item
  Helpful resource: \url{https://imaging.mrc-cbu.cam.ac.uk/statswiki/FAQ/effectSize}
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{etaSquared}\NormalTok{(TwoWay\_neg, }\AttributeTok{anova =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(effectsize)}
\FunctionTok{eta2\_to\_f}\NormalTok{(}\FloatTok{0.1066}\NormalTok{) }\CommentTok{\#FactorA {-}{-} Rater}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{eta2\_to\_f}\NormalTok{(}\FloatTok{0.1274}\NormalTok{) }\CommentTok{\#Factor B {-}{-} Photo}
\end{Highlighting}
\end{Shaded}
...
Now, to calculate power for our existing model. We'll use the package \emph{pwr2} and the function \emph{pwr.2way()}. To specify this we identify:
...
\FunctionTok{pwr.2way}\NormalTok{(}\AttributeTok{a=}\DecValTok{3}\NormalTok{, }\AttributeTok{b=}\DecValTok{2}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.05}\NormalTok{, }\AttributeTok{size.A =} \DecValTok{37}\NormalTok{, }\AttributeTok{size.B =} \DecValTok{55}\NormalTok{, }\AttributeTok{f.A =}\NormalTok{ .}\DecValTok{345}\NormalTok{, }\AttributeTok{f.B =}\NormalTok{ .}\DecValTok{382}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\textbf{Calculating sample size requirements}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ss.2way}\NormalTok{(}\AttributeTok{a =} \DecValTok{3}\NormalTok{, }\AttributeTok{b =} \DecValTok{2}\NormalTok{, }\AttributeTok{alpha =}\NormalTok{ .}\DecValTok{05}\NormalTok{, }\AttributeTok{beta =}\NormalTok{ .}\DecValTok{8}\NormalTok{, }\AttributeTok{f.A =}\NormalTok{ .}\DecValTok{345}\NormalTok{, }\AttributeTok{f.B =}\NormalTok{ .}\DecValTok{382}\NormalTok{, }\AttributeTok{B=} \DecValTok{100}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Often times researchers will play around with the \emph{f} values. Remember Cohen's indication of small (.10), medium (.25), and large (.40). We know we have a small effect for race and a larger effect for neighborhood income. Let's see what happens when we enter different values. Specifically, what if we only had a medium effect.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ss.2way}\NormalTok{(}\AttributeTok{a =} \DecValTok{3}\NormalTok{, }\AttributeTok{b =} \DecValTok{2}\NormalTok{, }\AttributeTok{alpha =}\NormalTok{ .}\DecValTok{05}\NormalTok{, }\AttributeTok{beta =}\NormalTok{ .}\DecValTok{80}\NormalTok{, }\AttributeTok{f.A =}\NormalTok{ .}\DecValTok{25}\NormalTok{, }\AttributeTok{f.B =}\NormalTok{ .}\DecValTok{25}\NormalTok{, }\AttributeTok{B=} \DecValTok{100}\NormalTok{) }\CommentTok{\#if we expected a medium effect}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ss.2way}\NormalTok{(}\AttributeTok{a =} \DecValTok{3}\NormalTok{, }\AttributeTok{b =} \DecValTok{2}\NormalTok{, }\AttributeTok{alpha =}\NormalTok{ .}\DecValTok{05}\NormalTok{, }\AttributeTok{beta =}\NormalTok{ .}\DecValTok{80}\NormalTok{, }\AttributeTok{f.A =}\NormalTok{ .}\DecValTok{10}\NormalTok{, }\AttributeTok{f.B =}\NormalTok{ .}\DecValTok{10}\NormalTok{, }\AttributeTok{B=} \DecValTok{100}\NormalTok{) }\CommentTok{\#if we expected a small effect}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{problem-2-conduct-a-one-way-anova-with-the-positive-evaluation-dependent-variable.}{%
\subsection{\texorpdfstring{Problem \#2: Conduct a one-way ANOVA with the \emph{positive evaluation} dependent variable.}{Problem \#2: Conduct a one-way ANOVA with the positive evaluation dependent variable.}}\label{problem-2-conduct-a-one-way-anova-with-the-positive-evaluation-dependent-variable.}}

The Ramdhani et al. \citeyearpar{ramdhani_affective_2018} article has two dependent variables (negative and positive evaluation) which are suitable for two-way ANOVAs. I used \emph{negative evaluation} as the dependent variable; you are welcome to conduct the analysis with \emph{positive evaluation} as the dependent variable.
...
In the Green and Salkind \citeyearpar{green_using_2014} statistics text, the one-way repeated measures example used in the lesson compared a teachers' perception of stress when responding to parents, teachers, and school administrators.
...
\hypertarget{id}{%
\centering
\includegraphics[width=10.41667in,height=1.04167in]{images/oneway_repeated/repeated_design.jpg}
\caption{Illustration of a research design appropriate for one-way repeated measures ANOVA}\label{id}
}
\end{figure}
...
Eight participants (seven transgender women and one genderqueer person) participated in the intervention. The mean age was 28.5 (\emph{SD} = 5.85). All participants were Italian.
...
The dependent variable (assessed at each wave) was a 14-item resilience scale \citep{wagnild_development_1993}. Items were assessed on a 7-point scale ranting from \emph{Strongly disagree} to \emph{Strongly agree} with higher scores indicating higher levels of resilience. An example items was, ``I usually manage one way or another.''
...
\CommentTok{\#OneWay\_mod\textless{}{-}aov(Resilience\textasciitilde{}Wave + Error(ID/(Wave)), AmodeoSIM)}
\CommentTok{\#summary(OneWay\_mod)}
\CommentTok{\#model.tables(OneWay\_mod,"means")}
\end{Highlighting}
\end{Shaded}
...
However, for some of the calculations (particularly those we will do by hand), we need the data to be in its typical wide form (formally called the \emph{person level} form). We can do this with the \emph{data.table} and \emph{reshape2}()* packages.
...
In the following output, see the order of presentation on the first screen of output. Even though we have ordered our factor so that ``Pre'' is first, the \emph{describeBy()} function seems to be ordering them alphabetically.
...
\begin{verbatim}
    item   group1 vars n     mean        sd   median  trimmed       mad
X11    1 FollowUp    1 8 6.136916 0.4729432 6.215983 6.136916 0.4416578
X12    2     Post    1 8 6.327615 0.6550520 6.356491 6.327615 0.8751431
X13    3      Pre    1 8 5.587693 0.8217561 5.770952 5.587693 1.1471137
         min      max    range       skew   kurtosis        se
X11 5.186767 6.708259 1.521491 -0.7204842 -0.6102953 0.1672107
X12 5.283057 7.089591 1.806534 -0.2307393 -1.6287654 0.2315959
X13 4.428839 6.597214 2.168376 -0.1437061 -1.8118551 0.2905347
\end{verbatim}
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-198-1.pdf}
...
\hypertarget{id}{%
\centering
\includegraphics[width=5.20833in,height=2.60417in]{images/oneway_repeated/SourceTable.jpg}
\caption{Demonstration of partitioning variance}\label{id}
}
\end{figure}
...
Let's take a moment to \emph{hand-calculate} \(SS_{T}\) (but in R).
...
Degrees of freedom (df) within is \emph{N - k}; or the summation of the df for each of the persons.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(psych)}
\FunctionTok{describeBy}\NormalTok{(Resilience }\SpecialCharTok{\textasciitilde{}}\NormalTok{ ID, }\AttributeTok{data =}\NormalTok{ Amodeo\_long, }\AttributeTok{mat =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{digits =} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
In this case, we are interested in change in resilience over time. Hence, \emph{time} is our mode. In our equation, we have three chunks representing the pre, post, and follow-up \emph{conditions} (waves). From each, we subtract the grand mean, square it, and multiply by the \emph{n} observed in each wave.

Degrees of freedom (df) for \(SS_M\) is \emph{k} - 1
...
Our example has 2 (numerator) and 14 (denominator) degrees of freedom. Rolling down to the table where \(\alpha = .05\), we can see that any \(F\) value \textgreater{} 3.73 will be statistically significant. Our \(F\) = 3.87, so we have (just barely) exceeded the thresshhold. This is our \emph{omnibus F}. We know there is at least 1 statistically significant difference between our pre, post, and follow-up conditions.
...
\item
  ...
  \tightlist
  \item
    Check by visualizing the data using box plots and the function \emph{identify\_outliers()} {[}rstatix{]}
  \item
...
\item
  The population variance of difference scores computed between any two levels of a within-subjects factor is the same value regardless of which two levels are chosen; termed the \textbf{sphericity assumption}.
...
Using the data from our motivating example, I calculated differences for each of the time variables. When we get into the analysis, we will use \emph{Mauchly's test} in hopes that there are non-significant differences in variances between all three of the comparisons.
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(ggpubr)}
\NormalTok{bxp }\OtherTok{\textless{}{-}} \FunctionTok{ggboxplot}\NormalTok{(Amodeo\_long, }\AttributeTok{x =} \StringTok{"Wave"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Resilience"}\NormalTok{, }\AttributeTok{add =} \StringTok{"point"}\NormalTok{, }\AttributeTok{xlab =} \StringTok{"Assessment Wave"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Self{-}Perception of Resilience"}\NormalTok{)}
\NormalTok{bxp}
\end{Highlighting}
\end{Shaded}

\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-216-1.pdf}
...
\NormalTok{Amodeo\_long }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{identify\_outliers}\NormalTok{(Resilience)}
\end{Highlighting}
\end{Shaded}
...
No outliers are identified. Visual inspection of boxplots, assisted by the \emph{identify\_outliers()} function in the \emph{rstatix} package (which reports values above Q3 + 1.5xIQR or below Q1 - 1.5xIQR, where IQR is the interquartile range) indicated no outliers.
...
Our skew and kurtosis values fall below the thresshholds of concern \citep{kline_principles_2016}:
...
We can use the Shapiro-Wilk test for a formal detection of normality. When \emph{p} \textless{} .05, it indicates that the distribution is statistically significantly different than a normal one. Therefore, \emph{p} \textgreater{} .05 indicates we did not violate the normal distribution assumption.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Amodeo\_long }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(Wave) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{shapiro\_test}\NormalTok{(Resilience)}
\end{Highlighting}
\end{Shaded}
...
Great! The \emph{p} value is \textgreater{} .05 for each of the cells.

The Shapiro-Wilk test is sensitive to sample size \citep{noauthor_repeated_nodate}. Samples \textgreater{} 50 may lead to \emph{p} values that are \textless{} .05, even when non-normality is not problematic. Therefore a second check with a QQ plot can be helpful.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggqqplot}\NormalTok{(Amodeo\_long, }\StringTok{"Resilience"}\NormalTok{, }\AttributeTok{facet.by =} \StringTok{"Wave"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-220-1.pdf}

We already have a non-significant \emph{p} from the Shapiro-Wilk; and these dots stay pretty well on the prediction line.
...
The sphericity assumption is automatically checked with Mauchley's test during the computation of the ANOVA when the \emph{anova\_test()} {[}rstatix package{]} is used. When the \emph{get\_anova\_table()} {[}rstatix{]} function is used, the Greenhouse-Geisser sphericity correction is automatically applied to factors violating the sphericity assumption.
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(rstatix)}
\NormalTok{RM\_AOV }\OtherTok{\textless{}{-}} \FunctionTok{anova\_test}\NormalTok{(}\AttributeTok{data =}\NormalTok{ Amodeo\_long, }\AttributeTok{dv =}\NormalTok{ Resilience, }\AttributeTok{wid =}\NormalTok{ ID, }\AttributeTok{within =}\NormalTok{ Wave)}
\NormalTok{RM\_AOV}
\end{Highlighting}
\end{Shaded}
...
If we needed to write it up, the \emph{F} string might look like this:
...
Note that when I am calculating these pairwise \emph{t} tests, I am creating an object (named ``pwc''). The object will be a helpful tool in creating a Figure and an APA Style table.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwc }\OtherTok{\textless{}{-}}\NormalTok{ Amodeo\_long }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{pairwise\_t\_test}\NormalTok{(Resilience}\SpecialCharTok{\textasciitilde{}}\NormalTok{Wave, }\AttributeTok{paired =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{p.adjust.method =} \StringTok{"bonferroni"}\NormalTok{)}
\NormalTok{pwc}
\end{Highlighting}
\end{Shaded}
...
What about Type I error? With only three possible post-omnibus comparisons, I will claim the Tukey LSD approach and not adjust the alpha to a more conservative level \citep{green_using_2014}.

Coolest boxplots ever

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwc }\OtherTok{\textless{}{-}}\NormalTok{ pwc }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{add\_xy\_position}\NormalTok{(}\AttributeTok{x =} \StringTok{"Wave"}\NormalTok{)}
\NormalTok{bxp }\SpecialCharTok{+} 
  \FunctionTok{stat\_pvalue\_manual}\NormalTok{(pwc) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{subtitle =} \FunctionTok{get\_test\_label}\NormalTok{(RM\_AOV, }\AttributeTok{detailed =} \ConstantTok{TRUE}\NormalTok{),}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(MASS)}
\FunctionTok{write.matrix}\NormalTok{(pwc, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"PWC.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\textbf{Let's compare this to the write-up in the Amodeo et al.\citeyearpar{amodeo_empowering_2018}article.}

\begin{itemize}
\tightlist
\item
  The \emph{F} string is presented in the Table 1 note (\emph{F}{[}1.612, 11.283{]}) = 6.390, \emph{p} = 0.18, \(\eta _{p}^{2}\))

  \begin{itemize}
  \tightlist
  \item
    we can tell from the df that the \emph{p} value has been had a correction for violation of the sphericity assumption
  \end{itemize}
\item
...
f = Cohen's \emph{f} (an effect size; we can use a conversion calculator)
nscor = the Greenhouse Geiser correction from our ouput; 1.0 means no correction was needed and is the package's default; \textless{} 1 means some correction was applied.
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(effectsize)}
\FunctionTok{eta2\_to\_f}\NormalTok{(.}\DecValTok{203}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(WebPower)}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{problem-2-change-the-random-seed}{%
\subsection{Problem \#2: Change the Random Seed}\label{problem-2-change-the-random-seed}}
...
Our analysis of the Amodio et al. \citeyearpar{amodio_empowering_2018} data failed to find significant increases in resilience from pre-to-post through follow-up. Our power analysis suggested that a sample size of 50 would be sufficient to garner statistical significance. The new dataset has been resimulated with this new sample size.
...
\hypertarget{problem-2-try-something-entirely-new}{%
\subsection{Problem \#2: Try Something Entirely New}\label{problem-2-try-something-entirely-new}}
...
\CommentTok{\#if(!require(tidyverse))\{install.packages("tidyverse")\}}
\CommentTok{\#if(!require(psych))\{install.packages("psych")\}}
\CommentTok{\#if(!require(jtools))\{install.packages("jtools")\}}
\end{Highlighting}
\end{Shaded}
...
\caption{Image of a flowchart and decision-tree for mixed design ANOVA}
\end{figure}
...
\item
  ...
  \tightlist
  \item
    Check with Shapiro-Wilk normality test \emph{shapiro\_test()} {[}rstatix{]} or visual inspection using the QQ plot (ggqqplot())*ggqqplot(){[}ggpubr{]})
  \end{itemize}
\item
  The variances of the differences between groups should be equal. This is termed the \textbf{sphericity assumption.} This can be checked with Mauchly's test of sphericity, which is reported automatically with \emph{anova\_test()}{[}rstatix{]}.
\end{itemize}
...
\item
  A robust ANOVA in the \emph{WRS2} package
\item
...
\CommentTok{\#Arab\_2way \textless{}{-} anova\_test(}
  \CommentTok{\#data = Murrar\_df, dv = AttArab, wid = caseID, \#UEdf is our df, dv is our DV, wid is the participant ID}
  \CommentTok{\#between = COND, within = Wave \# between is the between{-}subjects variable, within is the within subjects variable}
\CommentTok{\#  )}
\CommentTok{\#Arab\_2way}

\CommentTok{\#Diff\textless{}{-}round(c(rnorm(98,mean=12.68,sd=15.57),rnorm(95,mean=11.00,sd=17.25),rnorm(98, mean=13.31, sd=16.87), rnorm(95, mean=5.29, \#sd=13.73), rnorm(98, mean=12.50, sd=16.24), rnorm(95, mean=5.60, sd=15.18)),3) \#sample size, M and SD for each cell; this will \#put it in a long file}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{CNDwiWV }\OtherTok{\textless{}{-}} \FunctionTok{ggboxplot}\NormalTok{(}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{WVwiCND }\OtherTok{\textless{}{-}} \FunctionTok{ggboxplot}\NormalTok{(}
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-244-1.pdf}
...
Using the \emph{rstatix} package we can look for outliers in the dependent variable, doubly grouped by our predictor variables.
...
\NormalTok{Murrar\_df }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{identify\_outliers}\NormalTok{(Diff)}
\end{Highlighting}
\end{Shaded}
...
Next we can use the \emph{shapiro\_test()} to see if any of the distributions of the dependent variable (Diff) within each wave-by-condition combinations differs significantly from a normal distribution.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggqqplot}\NormalTok{(Murrar\_df, }\StringTok{"Diff"}\NormalTok{, }\AttributeTok{ggtheme =} \FunctionTok{theme\_bw}\NormalTok{()) }\SpecialCharTok{+}  \FunctionTok{facet\_grid}\NormalTok{(Wave }\SpecialCharTok{\textasciitilde{}}\NormalTok{ COND)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{Murrar\_df }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(Wave) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{levene\_test}\NormalTok{(Diff }\SpecialCharTok{\textasciitilde{}}\NormalTok{ COND)}
\end{Highlighting}
\end{Shaded}
...
\caption{Image of a flowchart showing that we are on the ``Compute the Omnibus ANOVA'' portion of the decision-tree}
\end{figure}
...
\caption{Image of a flowchart showing that we are on the ``Simple Main Effects for Factor A within all levels of Factor B'' portion of the decision-tree}
\end{figure}
...
\NormalTok{SimpleWave }\OtherTok{\textless{}{-}}\NormalTok{ Murrar\_df }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\#crate an object to hold the output}
  ...
  \FunctionTok{anova\_test}\NormalTok{(}\AttributeTok{dv =}\NormalTok{ Diff, }\AttributeTok{wid =}\NormalTok{ caseID, }\AttributeTok{between =}\NormalTok{ COND) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\#the between = Cond means that each level of cond will be compared}
  \FunctionTok{get\_anova\_table}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{adjust\_pvalue}\NormalTok{(}\AttributeTok{method =} \StringTok{"bonferroni"}\NormalTok{) }\CommentTok{\#we will get both the standard and adjusted p values}
\end{Highlighting}
\end{Shaded}
...
\item
  FollowUp: \emph{F} (1, 191) = 5.994, \emph{p} = .015, \(\eta^{2}\) = 0.030 (a small effect size)
\end{itemize}

Recall, interpretation for the eta-squared are .02 \textasciitilde{} small, .13 \textasciitilde{} medium, \textgreater.26 \textasciitilde{} large
...
As always, we have several choices about how to manage Type I error. In a circumstance when the analysis of simple main effects (condition within wave) includes only three pairwise comparisons, we can use the LSD method \citep{green_using_2014}. This means that we can we can leave the alpha at 0.05. If we were to use a traditional Bonferroni, we would use \(\alpha\) = .017 (.05/3). Although the more restrictive Bonferonni criteria comes close, in both cases we would still have one non-significant(baseline) and two significant (post1, post2) simple main effects.
...
\caption{Image of a flowchart showing that we are on the ``Simple Main Effects for Factor B within all levels of Factor A'' portion of the decision-tree}
\end{figure}
...
\NormalTok{SimpleCond }\OtherTok{\textless{}{-}}\NormalTok{ Murrar\_df }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(COND) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{anova\_test}\NormalTok{(}\AttributeTok{dv =}\NormalTok{ Diff, }\AttributeTok{wid =}\NormalTok{ caseID, }\AttributeTok{within =}\NormalTok{ Wave) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{get\_anova\_table}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
...
\caption{Image of a flowchart showing that we are on the ``Main effects only'' portion of the decision-tree}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwcWVwiGP }\OtherTok{\textless{}{-}}\NormalTok{ pwcWVwiGP }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{add\_xy\_position}\NormalTok{(}\AttributeTok{x =} \StringTok{"Wave"}\NormalTok{)}
\NormalTok{CNDwiWV }\SpecialCharTok{+} 
  \FunctionTok{stat\_pvalue\_manual}\NormalTok{(pwcWVwiGP, }\AttributeTok{tip.length =} \DecValTok{0}\NormalTok{, }\AttributeTok{hide.ns =} \ConstantTok{TRUE}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{subtitle =} \FunctionTok{get\_test\_label}\NormalTok{(Diff\_2way, }\AttributeTok{detailed =} \ConstantTok{TRUE}\NormalTok{),}
    \AttributeTok{caption =} \FunctionTok{get\_pwc\_label}\NormalTok{(pwcWVwiGP)}
\NormalTok{  )}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwcWVwiGP }\OtherTok{\textless{}{-}}\NormalTok{ pwcWVwiGP }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{add\_xy\_position}\NormalTok{(}\AttributeTok{x =} \StringTok{"Wave"}\NormalTok{) }\CommentTok{\#pwcWVwiGP were my pairwise comparisons for the simple effect}
\NormalTok{WVwiCND }\SpecialCharTok{+}  \CommentTok{\#WVwiCND was the boxplot before I did the ANOVA}
  \FunctionTok{stat\_pvalue\_manual}\NormalTok{(pwcWVwiGP, }\AttributeTok{tip.length =} \DecValTok{0}\NormalTok{, }\AttributeTok{hide.ns =} \ConstantTok{TRUE}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{subtitle =} \FunctionTok{get\_test\_label}\NormalTok{(Diff\_2way, }\AttributeTok{detailed =} \ConstantTok{TRUE}\NormalTok{), }\CommentTok{\#UE\_2way was my omnibus ANOVA model}
    \AttributeTok{caption =} \FunctionTok{get\_pwc\_label}\NormalTok{(pwcWVwiGP) }\CommentTok{\#and again the pairwise comparisons for the simple effect}
\NormalTok{  )}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(MASS)}
\FunctionTok{write.matrix}\NormalTok{(pwcWVwiGP, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"pwcWVwiGP.csv"}\NormalTok{)}
\CommentTok{\#this command can also be used to export other output}
\FunctionTok{write.matrix}\NormalTok{(Diff\_2way}\SpecialCharTok{$}\NormalTok{ANOVA, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"Diff\_2way.csv"}\NormalTok{) }\CommentTok{\#can get name of specific part of object by using str(object)}
\FunctionTok{write.matrix}\NormalTok{(SimpleWave, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"SimpleWave.csv"}\NormalTok{)}
\FunctionTok{write.matrix}\NormalTok{(SimpleCond, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"SimpleCond.csv"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
f = Cohen's \emph{f} (an effect size; we can use a conversion calculator); Cohen suggests that f values of 0.1, 0.25, and 0.4 represent small, medium, and large effect sizes, respectively.
nscor = the Greenhouse Geiser correction from our ouput; 1.0 means no correction was needed and is the package's default; \textless{} 1 means some correction was applied.
alpha = is the probability of Type I error; we traditionally set this at .05
power = 1 - P(Type II error) we traditionally set this at .80 (so anything less is less than what we want)
type = 0 is for between-subjects, 1 is for repeated measures, \textbf{2 is for interaction effect}.

As in the prior lessons, we need to convert our effect size for the \emph{interaction} to \(f\) effect size (this is not the same as the \emph{F} test). The \emph{effectsize} package has a series of converters. We can use the \emph{eta2\_to\_f()} function.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(effectsize)}
\FunctionTok{eta2\_to\_f}\NormalTok{(}\FloatTok{0.017}\NormalTok{) }\CommentTok{\#interaction effect}
\end{Highlighting}
\end{Shaded}
...
6. APA style results with table(s) and figure & 5 & \_\_\_\_\_ \\
7. Explanation to grader & 5 & \_\_\_\_\_ \\
8. What part of the simulation did you change? What did you have to do to get a significant interaction effect? & 5 & \_\_\_\_\_ \\
\textbf{Totals} & 35 & \_\_\_\_\_ \\
\bottomrule
\end{longtable}
...
\item
  ...
  \end{itemize}
\item
  Bernerth, J. B., \& Aguinis, H. (2016). A critical review and best‐practice recommendations for control variable usage. Personnel Psychology, 69(1), 229--283. \url{https://doi.org/10.1111/peps.12103}
...
\CommentTok{\#if(!require(data.table))\{install.packages("data.table")\}\#used to convert data from long to wide}
...
\CommentTok{\#if(!require(WebPower))\{install.packages("WebPower")\} \#power analysis for repeated measures}
\end{Highlighting}
\end{Shaded}
...
Green and Salkind \citep{green_one-way_2014-1} identified common uses of ANCOVA:
...
\textbf{Normally distributed} The DV is normally distributed in the population for any specific value of the covariate and for any one level of a factor. This assumption applies to every combination of the values of the covariate and levels of the factor and requires them all to be normally distributed. To the degree that population distributions are not normal and sample sizes are small, \emph{p} values may not be trustworthy and power reduced. Evaluating this is frequently operationalized by inspecting the residuals and identifying outliers.
...
We are following the approach to analyzing ANCOVA identifed in the Datanovia lesson on ANCOVA \citep{noauthor_ancova_nodate}.
...
\caption{Image of the ANCOVA decision-tree and workflow}
\end{figure}
...
\caption{Image of the ANCOVA decision tree, showing our current place in the workflow.}
\end{figure}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(ggpubr)}
\FunctionTok{ggscatter}\NormalTok{ (}
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-273-1.pdf}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(rstatix)}
\NormalTok{Murrar\_wide }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{anova\_test}\NormalTok{(AttArabP1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ COND}\SpecialCharTok{*}\NormalTok{AttArabB)}
\end{Highlighting}
\end{Shaded}
...
Preliminary analysis did not support ANCOVA as a statistical option in that there was a violation of the homogeneity of regression slopes as the interaction term was statistically significant, \emph{F} (1, 189) = 4.297, \emph{p} = .040, \(\eta^2\) = 0.022. We should probably stop here. However, for the sake of demonstration, I will continue. One of the reasons I wanted to work this example as ANCOVA is to demonstrate that covariates and moderators each have their role. We can already see how this data is best analyzed with mixed design ANOVA.
...
Once we have the residuals, we can treat them as data and evaluate if they are normally distributed. We first compute the model with \emph{lm()} (lm stands for ``linear model''). This is a linear regression.
...
From the \emph{broom} package (with the \emph{augment(model)} function) we can augment our \emph{lm()} model object to add fitted values and residuals.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(broom)}
\NormalTok{AttArabB\_Mod.metrics }\OtherTok{\textless{}{-}} \FunctionTok{augment}\NormalTok{(AttArabB\_Mod) }\CommentTok{\#new model by augmenting the lm model}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{shapiro\_test}\NormalTok{(AttArabB\_Mod.metrics}\SpecialCharTok{$}\NormalTok{.resid)}\CommentTok{\#apply shapiro\_test to that augmented model}
\end{Highlighting}
\end{Shaded}
...
Yikes again! The statistically significant Shapiro Wilk test indicated a violation of the normality assumption (\emph{W} = 0.984, \emph{p} = .026).
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{AttArabB\_Mod.metrics }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{levene\_test}\NormalTok{(.resid }\SpecialCharTok{\textasciitilde{}}\NormalTok{ COND) }
\end{Highlighting}
\end{Shaded}
...
\NormalTok{MurrarB\_ANCOVA }\OtherTok{\textless{}{-}}\NormalTok{ Murrar\_wide }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{anova\_test}\NormalTok{(AttArabP1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ AttArabB }\SpecialCharTok{+}\NormalTok{ COND)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{get\_anova\_table}\NormalTok{(MurrarB\_ANCOVA )}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{pwc\_B }\OtherTok{\textless{}{-}}\NormalTok{ Murrar\_wide }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{emmeans\_test}\NormalTok{(}
...
With the script below we can obtain the covariate-adjusted marginal means. These are termed \textbf{estimated marginal means.} Take a look at these and compare them to what we would see in the regular descriptives. It is helpful to see the grand mean (AttArabB) and then the marginal means (emmean).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{emmeans\_B }\OtherTok{\textless{}{-}} \FunctionTok{get\_emmeans}\NormalTok{(pwc\_B)}
...
Note that the \emph{emmeans} process produces slightly different means than the raw means from the \emph{psych} package's \emph{describeBy()} function. Why? Because the \emph{get\_emmeans()} function uses the model that included the covariate. The \emph{estimated} means are covariate-adjusted.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(psych)}
\NormalTok{descripts\_P1 }\OtherTok{\textless{}{-}} \FunctionTok{describeBy}\NormalTok{(AttArabP1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ COND, }\AttributeTok{data =}\NormalTok{ Murrar\_wide, }\AttributeTok{mat =} \ConstantTok{TRUE}\NormalTok{)}
...
In our case the adjustments are very minor. Why? The effect of the attitudes toward Arabs baseline test on the attitudes toward Arabs post test was nonsignificant. In fact, our correlation is\ldots{}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#getting p values requires the corr.test function from the psych package; }
\CommentTok{\#because some function names appear is multiple packages, it is sometimes essential to point a function to the proper package}
\CommentTok{\#use the double colon to specify package and function}
...
\ldots almost negligible (\emph{r} = -0.05, \emph{p} = .47)
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(MASS)}
\FunctionTok{write.matrix}\NormalTok{(pwc\_B, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"pwc\_B.csv"}\NormalTok{)}
\FunctionTok{write.matrix}\NormalTok{(emmeans\_B, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"emmeans\_B.csv"}\NormalTok{)}
\FunctionTok{write.matrix}\NormalTok{(descripts\_P1, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"descripts\_P1.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwc\_B }\OtherTok{\textless{}{-}}\NormalTok{ pwc\_B }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{add\_xy\_position}\NormalTok{(}\AttributeTok{x =} \StringTok{"COND"}\NormalTok{, }\AttributeTok{fun =} \StringTok{"mean\_se"}\NormalTok{)}
\FunctionTok{ggline}\NormalTok{(}\FunctionTok{get\_emmeans}\NormalTok{(pwc\_B), }\AttributeTok{x =} \StringTok{"COND"}\NormalTok{, }\AttributeTok{y =} \StringTok{"emmean"}\NormalTok{) }\SpecialCharTok{+}
  ...
  \FunctionTok{stat\_pvalue\_manual}\NormalTok{(pwc\_B, }\AttributeTok{hide.ns =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{tip.length =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{subtitle =} \FunctionTok{get\_test\_label}\NormalTok{(MurrarB\_ANCOVA, }\AttributeTok{detailed =} \ConstantTok{TRUE}\NormalTok{),}
    \AttributeTok{caption =} \FunctionTok{get\_pwc\_label}\NormalTok{(pwc\_B)}
\NormalTok{  )}
\end{Highlighting}
\end{Shaded}

\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-286-1.pdf}
\textbf{Results}
...
\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-287-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Murrar\_wide }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{anova\_test}\NormalTok{(AttArabP1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ COND}\SpecialCharTok{*}\NormalTok{AttWhiteP1)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(broom)}
\NormalTok{WhCov\_mod.metrics }\OtherTok{\textless{}{-}} \FunctionTok{augment}\NormalTok{(WhCov\_mod)}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{shapiro\_test}\NormalTok{(WhCov\_mod.metrics}\SpecialCharTok{$}\NormalTok{.resid)}
\end{Highlighting}
\end{Shaded}
...
Yikes! The statistically significant Shapiro Wilk test indicate a violation of the normality assumption (\emph{W} = 0.984, \emph{p} = .029). As I mentioned before, there are better ways to analyze this research vignette. I am only modeling this as an alternative.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{WhCov\_mod.metrics }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{levene\_test}\NormalTok{(.resid }\SpecialCharTok{\textasciitilde{}}\NormalTok{ COND)}
\end{Highlighting}
\end{Shaded}
...
Yikes again. A statistically significant Levene's test indicates a violation of the homogeneity of the residual variances (\emph{F}{[}1, 191{]} = 4.539, \emph{p} = .034).
...
\NormalTok{WhCov\_ANCOVA }\OtherTok{\textless{}{-}}\NormalTok{ Murrar\_wide }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{anova\_test}\NormalTok{(AttArabP1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ AttWhiteP1 }\SpecialCharTok{+}\NormalTok{ COND)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{get\_anova\_table}\NormalTok{(WhCov\_ANCOVA)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{emmeans\_cond }\OtherTok{\textless{}{-}} \FunctionTok{get\_emmeans}\NormalTok{(pwc\_cond)}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(MASS)}
\FunctionTok{write.matrix}\NormalTok{(pwc\_cond, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"pwc\_con.csv"}\NormalTok{)}
\FunctionTok{write.matrix}\NormalTok{(emmeans\_cond, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"emmeans\_con.csv"}\NormalTok{)}
\FunctionTok{write.matrix}\NormalTok{(descripts\_cond, }\AttributeTok{sep =} \StringTok{","}\NormalTok{, }\AttributeTok{file =} \StringTok{"descripts\_con.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pwc\_cond }\OtherTok{\textless{}{-}}\NormalTok{ pwc\_cond }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{add\_xy\_position}\NormalTok{(}\AttributeTok{x =} \StringTok{"COND"}\NormalTok{, }\AttributeTok{fun =} \StringTok{"mean\_se"}\NormalTok{)}
\FunctionTok{ggline}\NormalTok{(}\FunctionTok{get\_emmeans}\NormalTok{(pwc\_B), }\AttributeTok{x =} \StringTok{"COND"}\NormalTok{, }\AttributeTok{y =} \StringTok{"emmean"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_errorbar}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{ymin =}\NormalTok{ conf.low, }\AttributeTok{ymax =}\NormalTok{ conf.high), }\AttributeTok{width =} \FloatTok{0.2}\NormalTok{) }\SpecialCharTok{+} 
  \FunctionTok{stat\_pvalue\_manual}\NormalTok{(pwc\_B, }\AttributeTok{hide.ns =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{tip.length =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{subtitle =} \FunctionTok{get\_test\_label}\NormalTok{(WhCov\_ANCOVA, }\AttributeTok{detailed =} \ConstantTok{TRUE}\NormalTok{),}
    \AttributeTok{caption =} \FunctionTok{get\_pwc\_label}\NormalTok{(pwc\_cond)}
\NormalTok{  )}
\end{Highlighting}
\end{Shaded}

\includegraphics{ReCenterPsychStats_files/figure-latex/unnamed-chunk-300-1.pdf}
\textbf{Results}
...
\hypertarget{problem-2-conduct-a-one-way-anova-with-the-dv-and-covariate-at-post2.}{%
\subsection{Problem \#2: Conduct a one-way ANOVA with the DV and covariate at post2.}\label{problem-2-conduct-a-one-way-anova-with-the-dv-and-covariate-at-post2.}}
