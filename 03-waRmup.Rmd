...
* First, I named the data object that global environment "df" (i.e., dataframe). This sentence could be rewritten for clarity; a word may be missing?
...
Manipulating the data is an important skill. I can use the *select()* function to obtain certain variable.  I want to select three variables that are continuous and then disaggregate them by race/ethnicity. 
...
```{r}
describeBy (df, df$RacEth, mat=TRUE) it may be helpful to specify what "mat=TRUE' means above; is it necessary? why?
```
